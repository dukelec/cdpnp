...
for k in last_list:
    ...
    t_mpk = last_list[k][3]
    ret, info = invoke_cmd_r(f'{cdbus_gui}/tools/cdg_cmd.py --dev {k} --cfg {cdbus_gui}/configs/{t_json}')
    if ret != 0:
        exit(-1)
    ...
    if c_ver != t_ver:
        ...
        if 'y' in yn.lower():
            print("upgrading............")
            ret = invoke_cmd(f'{cdbus_gui}/tools/cdg_iap.py --dev {k} --cfg {cdbus_gui}/configs/{t_json} --in-file firmware/{t_hex}')
            if ret != 0:
                exit(-1)
            sleep(4)
            print("\n")
    ...
    ret = invoke_cmd(f'{cdbus_gui}/tools/cdg_cmd.py --dev {k} --cfg {cdbus_gui}/configs/{t_json} --import cdbus_gui_configs/{t_mpk}')
    if ret != 0:
        exit(-1)
    yn = input('Save config to flash, Y or N? (default N) ')
    if 'y' in yn.lower():
        ret = invoke_cmd(f'{cdbus_gui}/tools/cdg_cmd.py --dev {k} --cfg {cdbus_gui}/configs/{t_json} --reg save_conf --val 1')
        if ret != 0:
            exit(-1)
    else:
        print('skip')
